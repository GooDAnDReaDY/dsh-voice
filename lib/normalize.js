...
export function applyJargonDictionary(text, customReplacements = []) {
  ...
  const dict = [...(customReplacements || []), ...DEFAULT_JARGON_DICTIONARY]
  let result = text
  for (const item of dict) {
    if (!item?.from || !item?.to) continue
    const escaped = item.from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const regex = new RegExp(`(?<![a-zA-Zа-яА-ЯёЁ0-9_-])${escaped}(?![a-zA-Zа-яА-ЯёЁ0-9_-])`, 'gi')
...
export function extractVoiceActions(text) {
  ...
  if (/^(отправь|отправить|send|send it|send message)$/i.test(lower)) {
    return { text: '', action: 'send' }
  }
  if (/^(отмена|cancel|discard)$/i.test(lower)) {
    return { text: '', action: 'cancel' }
  }
  if (/^(очисти|очистить|clear|clear composer)$/i.test(lower)) {
    return { text: '', action: 'clear' }
  }
  if (/^(новая строка|с новой строки|new line|newline)$/i.test(lower)) {
    return { text: '', action: 'newline' }
  }

  const trailingSend = /[\s,]+(отправь|отправить|send|send it)[.!?…]*$/i
  if (trailingSend.test(raw)) {
    return { text: raw.replace(trailingSend, '').trim(), action: 'send' }
  }
  const trailingCancel = /[\s,]+(отмена|cancel)[.!?…]*$/i
  if (trailingCancel.test(raw)) {
    return { text: raw.replace(trailingCancel, '').trim(), action: 'cancel' }
  }
  const trailingNewline = /[\s,]+(новая строка|с новой строки|new line)[.!?…]*$/i
