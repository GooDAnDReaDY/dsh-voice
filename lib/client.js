...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VoiceSection(props) {
      ...
      const scope = ((ctx.get && ctx.get('lanSettings')) || ctx.configForms).get(NS)
      ...
      React.useEffect(() => {
        ...
        const timer = setInterval(() => {
          ...
          tries += 1
          try { ctx.configForms.describe().load() } catch (e) { /* service not up yet */ }
        }, 1000)
        ...
        React.createElement('div', { className: 'cb-section-card' },
          ...
          React.createElement('div', { className: 'cb-row', style: { alignItems: 'center', gap: '12px' } },
            React.createElement('span', { className: 'cb-badge cb-badge-ok' },
              'v' + ((updaterStatus && updaterStatus.currentVersion) || '0.8.32')
            ),
