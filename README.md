...
- id: dsh-voice
  config:
    dictation:
      chain:
        - provider: browser
        - provider: local-whisper   # fallback if browser cannot
```
...
Settings → **Plugins → Plugin settings → Voice** — the plugin's own collapsible card in the plugins tab; the sidebar keeps no separate row for it. The card has four blocks:
...
The local provider needs a running [whisper.cpp](https://github.com/ggerganov/whisper.cpp) server:
...
```
lib/index.js       хост: конфигурация, маршруты, transcribe_audio, автозапуск whisper
lib/providers.js   четыре провайдера, чистые функции (внедрение сети)
lib/chain.js       обход цепочки фолбеков
lib/wav.js         конвертация webm/opus → WAV 16 кГц моно через ffmpeg
lib/client.js      браузерная часть: кнопки ввода, запись, страница настроек
test/              модульные тесты node --test для цепочки и провайдеров
```
...
```
lib/index.js       服务端：配置、路由、transcribe_audio 工具、whisper 进程管理
lib/providers.js   四大核心服务商及预设实现（纯函数，支持网络注入）
lib/chain.js       备用链顺序轮询执行器
lib/wav.js         利用 ffmpeg 进行 webm/opus → WAV 16 kHz 单声道转码
lib/client.js      前端浏览器端：输入框按钮、录音条、设置面板 UI
test/              node --test 单元测试（备用链与各服务商接口解析）
```
